...
public class Listeners extends Base_function implements ITestListener
{
	static Date d = new Date();
	static String fileName = "ExtentReport_" + d.toString().replace(":", "_").replace(" ", "_") + ".html";

	private static ExtentReports extent = Extent_report.createInstance(System.getProperty("user.dir")+"\\reports\\"+fileName);
	...
	public void onTestFailure(ITestResult result) {
		...
		try {
			...
			Extent_report.captureScreenshot();
			testReport.get().fail("<b>" + "<font color=" + "red>" + "Screenshot of failure" + "</font>" + "</b>",
					MediaEntityBuilder.createScreenCaptureFromPath(Extent_report.screenshotName)
					.build());
		} catch (Exception e) {
